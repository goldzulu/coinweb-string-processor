...
import { STRING_PROCESSOR_TABLE, CONTRACT_ID } from './constants';
import * as CwebWallet from '@coinweb/wallet-lib';
import * as bip39 from 'bip39';
import createHmac from 'create-hmac/browser';
import { publicKeyCreate } from 'secp256k1';
import type { ClaimsResponse, IssuedClaim } from './types';

const DEV_COINWEB_ENDPOINT: string =
  typeof window === 'undefined' ? process.env.API_ENDPOINT_DEVNET : window.__API_URL__;
    ...
    calls: [
      {
        ...
        contract_ref: {
          explicit: [],
          stored: [constructSingleReadClaim(constructContractIssuer(CONTRACT_ID), constructSelfRegisterKey())],
        },
      } as ContractCall,
    ],
  };
...
export const prepareWallet = async (network: NetworkName): Promise<CwebWallet.Wallet> => {
...
export const prepareTransaction = async (callOp: CwebWallet.CustomUiCommand): Promise<CwebWallet.L2TransactionData> => {
  ...
  return await CwebWallet.compose_ui_commands(wallet, [{ CustomV1: callOp }], NetworkName.DEVNET_L1A);
};
